...
class DIORDataset(CustomDataset):
    ...
    def repeat_to_length(self, target_len):
        if len(self.data_infos) < target_len:
            repeat_times = math.ceil(target_len / len(self.data_infos))
            self.data_infos = self.data_infos * repeat_times
            self.data_infos = self.data_infos[:target_len]
