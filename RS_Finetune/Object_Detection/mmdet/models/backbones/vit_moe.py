...
from mmcv.cnn import Linear, build_norm_layer, build_activation_layer
...
class ViT_MOE(BaseModule):
    ...
    def load_moe_ffn_weights(self, pretrained_state_dict, verbose=True):
        """
        Args:
            model: 当前使用 FFNMOE 的模型
            pretrained_state_dict: 原始 FFN 模型的权重 dict
            part_features: MoE 输出中专家模块负责的维度大小
            verbose: 是否打印加载日志
        """
        for name, module in self.named_modules():
            ...
            # 构造原始 FFN 权重的 key 前缀（假设结构保持一致）
            # 例如 name = 'encoder.layers.3.ffn'
            ...
            # 原始 FFN 的 fc2 权重
            fc2_weight = pretrained_state_dict[old_fc2_weight_key]  # shape: [768, 3072]
            fc2_bias = pretrained_state_dict[old_fc2_bias_key]      # shape: [768]

            # 1. 分割 shared 输出部分
            shared_out_dim = module.embed_dims - module.part_features
            shared_weight = fc2_weight[:shared_out_dim, :]  # shape: [768 - part, 3072]
            shared_bias = fc2_bias[:shared_out_dim]         # shape: [768 - part]

            module.layers[-2].weight.data.copy_(shared_weight)
            module.layers[-2].bias.data.copy_(shared_bias)

            # 2. 分割 expert 输出部分，并复制给所有专家
            expert_weight = fc2_weight[shared_out_dim:, :]   # shape: [part, 3072]
            expert_bias = fc2_bias[shared_out_dim:]          # shape: [part]
    ...
    def init_weights(self, pretrained):
        ...
        if isinstance(pretrained, str):
            ...
            for k, v in state_dict.items():
                if k in model_dict and model_dict[k].shape == v.shape:
                    ...
                    if rank == 0:
                        print(f"[WARN] Skip loading: {k}, shape mismatch or not found")
        ...
        # print("\n".join(list(state_dict.keys())))
            if any(isinstance(m, FFNMOE) for m in self.modules()):
                self.load_moe_ffn_weights(state_dict)
        # if rank == 0:
            # print(f"Loaded state_dict with message: {msg}")
