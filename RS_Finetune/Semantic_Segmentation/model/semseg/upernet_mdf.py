...
class UperNet(torch.nn.Module):
    def __init__(self, args, cfg):
        ...
        self.semseghead_vaihingen = nn.Sequential(nn.Dropout2d(0.1), nn.Conv2d(256, cfg['vaihingen']['nclass'], 1))
        self.semseghead_potsdam = nn.Sequential(nn.Dropout2d(0.1), nn.Conv2d(256, cfg['potsdam']['nclass'], 1))
        self.semseghead_OpenEarthMap = nn.Sequential(nn.Dropout2d(0.1), nn.Conv2d(256, cfg['openearthmap']['nclass'], 1))
        self.semseghead_loveda = nn.Sequential(nn.Dropout2d(0.1), nn.Conv2d(256, cfg['loveda']['nclass'], 1))

    def forward(self, x, dataset='vaihingen'):
        b, c, h, w = x.shape
        if "moe" in self.args.backbone:
